...
def custom_loss(y_true, y_pred):
    import numpy as np
    eps = 10**(-4)   
    # print(y_true.shape)#reshape 
    y_true_r=tf.reshape(y_true,(-1,1))
    y_pred_r=tf.reshape(y_pred,(-1,1))    
    # y_true_r=y_true
    # y_pred_r=y_pred     
    # print(y_t_r.shape)
    y_t=tf.math.divide(y_true_r,tf.math.reduce_sum(y_true_r))
    y_p=tf.math.divide(y_pred_r,tf.math.reduce_sum(y_pred_r))
    y_p=tf.math.add(y_p,eps)
    y_t=tf.math.add(y_t,eps)
    # kl = lambda x,y: x * tf.math.log(x / y) - x + y
    kl = lambda x,y: tf.math.add(tf.math.subtract(tf.math.multiply(x,tf.math.divide(x, y)),x),y)

   
    return tf.math.reduce_sum(kl(y_t, y_p))
    # return tf.math.reduce_sum(kl(nt_hist, np_hist))
################
